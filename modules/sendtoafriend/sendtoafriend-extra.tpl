      ...
      <div class="row">
        ...
        <div class="product clearfix col-xs-12 col-sm-6">
          <div class="thumbnail" style="width: {$homeSize.width|intval}px; height: {$homeSize.height|intval}px">
            {if !empty($lazy_load)}
              <noscript>
                <img src="{$link->getImageLink($stf_product->link_rewrite, $stf_product_cover, 'home', null, ImageManager::retinaSupport())|escape:'url':'UTF-8'}"
            ...
            <picture class="img-responsive{if !empty($lazy_load)}tb-lazy-image{/if}" style="width: {$homeSize.width|intval}px; height: {$homeSize.height|intval}px">
              ...
              {if !empty($webp)}
                <source {if !empty($lazy_load)}srcset="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII= 1w" data-{/if}srcset="{$link->getImageLink($stf_product->link_rewrite, $stf_product_cover, 'home', 'webp', ImageManager::retinaSupport())|escape:'url':'UTF-8'}"
                        type="image/webp"
                >
              {/if}
              <!--[if IE 9]></video><![endif]-->
              <img {if !empty($lazy_load)}srcset="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII= 1w" data-{/if}srcset="{$link->getImageLink($stf_product->link_rewrite, $stf_product_cover, 'home', null, ImageManager::retinaSupport())|escape:'url':'UTF-8'}"
