            ...
            <div id="scene_products_cluetip_{$scene_key}_{$product_key}_{$product.id_product}" style="display:none;">
              <div class="product-image-container">
                {if isset($imageIds)}
                  {if !empty($lazy_load)}
                    <noscript>
                      <img src="{$link->getImageLink($product.id_product, $imageIds, 'home', null, ImageManager::retinaSupport())|escape:'url':'UTF-8'}"
                           alt=""
                  ...
                  <picture class="img-responsive{if !empty($lazy_load)} tb-lazy-image{/if}">
                    ...
                    {if !empty($webp)}
                      <source {if !empty($lazy_load)}srcset="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII= 1w" data-{/if}srcset="{$link->getImageLink($product.id_product, $imageIds, 'home', 'webp', ImageManager::retinaSupport())|escape:'url':'UTF-8'}"
                              type="image/webp"
                      >
                    {/if}
                    <!--[if IE 9]></video><![endif]-->
                    <img {if !empty($lazy_load)}srcset="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII= 1w" data-{/if}srcset="{$link->getImageLink($product.id_product, $imageIds, 'home', null, ImageManager::retinaSupport())|escape:'url':'UTF-8'}"
