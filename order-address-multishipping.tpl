...
<div class="addresses clearfix">
  ...
  {else}
    <a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn btn-success"><span>{l s='Add a new address'} <i class="fas fa-chevron-right"></i></span></a>
  {/if}
  </p>
  <div class="row">
  ...
  <p class="address_add submit">
    <a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back|urlencode}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn btn-success"><span>{l s='Add a new address'} <i class="fas fa-chevron-right"></i></span></a>
  </p>
  {if !$opc}
...
{if !$opc}
    <p class="cart_navigation clearfix">
      ...
      {if $back}
        <a href="{$link->getPageLink('order', true, NULL, "back={$back}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn btn-lg btn-default"><i class="fas fa-chevron-left"></i> {l s='Continue Shopping'}</a>
      {else}
        <a href="{$link->getPageLink('order', true, NULL)|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn btn-lg btn-default"><i class="fas fa-chevron-left"></i> {l s='Continue Shopping'}</a>
      {/if}
      <button type="submit" name="processAddress" class="btn btn-lg btn-success pull-right"><span>{l s='Proceed to checkout'} <i class="fas fa-chevron-right"></i></span></button>
    </p>
  </form>
{else}
  </div>
...
{strip}
  ...
  {capture}<a class="btn btn-success" href="{$smarty.capture.addressUrlAdd}" title="{l s='Update' js=1}"><span>{l s='Update' js=1} <i class="fas fa-chevron-right"></i></span></a>{/capture}
